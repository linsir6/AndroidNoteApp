...
public class PictureAndTextEditorView extends AppCompatEditText {
    ...
    public List<String> getmContentList() {
        ...
        String[] content = getText().toString().split(mNewLineTag);
        for (int i = 0; i < content.length; i++) {
            if (content[i].indexOf("/storage") != -1) {
                content[i] += mBitmapTag;
            }
...
//        }
    ...
    public Bitmap getSmallBitmap(String filePath, int reqWidth, int reqHeight) {
        ...
        bitmap = Bitmap.createScaledBitmap(bitmap, w_width / 2, w_height / 2, false);
        return bitmap;
    }
